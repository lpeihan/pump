...
export const POOL_LPMINT_SEED = Buffer.from(anchor.utils.bytes.utf8.encode('pool_lp_mint'));
...
export async function getAuthAddress(programId: PublicKey): Promise<[PublicKey, number]> {
  const [address, bump] = await PublicKey.findProgramAddress([POOL_AUTH_SEED], programId);
  return [address, bump];
}
